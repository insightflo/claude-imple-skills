...
function main() {
  ...
  const programBase = path.basename(String(program || '')).toLowerCase();
  const isGemini = programBase === 'gemini' || programBase.startsWith('gemini-');

  // Most CLIs accept a positional prompt argument, but Gemini CLI requires -p/--prompt.
  // If the config didn't include it, auto-inject it so the worker runs headless.
  const wantsPromptFlag = isGemini && !(args.includes('-p') || args.includes('--prompt'));
  const spawnArgs = wantsPromptFlag ? [...args, '-p', prompt] : [...args, prompt];
