...
- stale_ref_count: 1 (프로젝트 소스에서 삭제된 에이전트/스킬 참조가 남아있는 파일 수)
- test_pass_rate: 100.0% (pre-existing 1건 제외 기준)
- dead_code_files: 1 (.claude/agents/ 에서 어디서도 참조되지 않는 에이전트)
- doc_consistency: 100
- **복합 점수: 68.0** (= 0*0.30 + 100*0.30 + 90*0.20 + 100*0.20)
...
- 주요 패턴: `ProjectManager`, `ChiefArchitect`, `ChiefDesigner`, `QAManager`, `BackendSpecialist`, `FrontendSpecialist`, `SecuritySpecialist`, `DBA.md`, `orchestrate-standalone`, `hook-shims`, `task-board`, `Lead.md`

### P1: Dead Agent 정리
`.claude/agents/`에 24개 에이전트 파일이 있지만, 실제 사용되는 것은:
- **Agent Teams 리더** (4): team-lead, architecture-lead, qa-lead, design-lead
- **Task 위임 대상**: test-specialist 등 (SKILL.md나 에이전트에서 Task() 대상으로 참조)

나머지(orchestrator, 3d-engine-specialist, api-designer, system-designer 등)는 사용 여부를 확인하고:
- 어디서도 참조되지 않으면 삭제
- 참조되면 유지

주의: `.claude/agents/`의 PascalCase 파일들(Builder.md, Reviewer.md, Designer.md, MaintenanceAnalyst.md)은 project-team/agents/의 복제 — 중복 제거 대상

### P2: 문서 숫자 일관성
- 모든 문서에서 에이전트 수, 훅 수, 스킬 수가 동일한지 확인
- 현재 정확한 수: **20 skills, 19 hooks, 4 leads + 4 workers**
...
## 힌트 (Outer Loop가 업데이트)
- (초기 상태 — 아직 실험 전)
- stale_ref_count=1: PROJECT-BOOTSTRAP-INTEGRATION.md에 잔존 가능성 높음
- dead_code_files=1: .claude/agents/에서 미참조 에이전트 1개 이상 존재
- PascalCase 중복 (Builder.md 등)이 .claude/agents/에 남아있음 — project-team/agents/와 동일 파일
